...
public class ThreadWeb implements Runnable {
	...
	public void run()
	{
		...
		try{
			...
			this.webHeaders=method.setWebHeadersers(headers);
			...
			for(Map.Entry<String,Map<String, String>> webParam: webParams.entrySet()) 	
			{   
				System.out.println(method.setUrl()+resourceList.get(0)); 
				Response resp=given().urlEncodingEnabled(false).log().all().contentType("application/json\r\n").when().parameters(webParam.getValue()).headers(webHeaders).get(method.setUrl()+resourceList.get(0));
				...
				//	mapper.put( webParam.getKey(), idPriotMap);
